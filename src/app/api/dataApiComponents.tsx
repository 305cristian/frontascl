...
export async function GetProgramas() {
    try {
        ...
        if (response.data) {
            ...
            const lista = data.map(({idPrograma, descriptionPrograma}: {
                idPrograma: number;
...
export async function GetLugarObs() {
    try {
        ...
        if (response.data) {
            const data = response.data.object;
            //alimento la constante lista
            const lista = data.map(({idLugarObser, nameLugarObservacion}: {
                idLugarObser: number;
    ...
    try {
        ...
        if (response.data) {
            ...
            const lista = data.map(
                ({idActLiderazAdic, descriptionActLiderazgoAdic}: {
                    idActLiderazAdic: number;
                    descriptionActLiderazgoAdic: string
                }) => ({
...
export async function GetActividadLiderazgo() {
    try {
        ...
        if (response.data) {
            const data = response.data.object;
            //alimento la constante lista
            const lista = data.map(({idListActLiderazgo, nameListActLiderazgo}: {
                idListActLiderazgo: number;
                nameListActLiderazgo: string
            }) => ({
